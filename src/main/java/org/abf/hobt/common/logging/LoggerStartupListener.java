...
public class LoggerStartupListener extends ContextAwareBase implements LoggerContextListener, LifeCycle {
    ...
    private String fetchLogDirectory() {
        ...
        String propertyHome = System.getenv("HOBT_LOG_HOME");
        if (!StringUtils.isBlank(propertyHome))
            return propertyHome;

        // check system property (-D in startup script)
        propertyHome = System.getProperty("HOBT_LOG_HOME");
        if (!StringUtils.isBlank(propertyHome))
            return propertyHome;

        // check tmp directory
        propertyHome = System.getProperty("java.io.tmpdir");
        if (!StringUtils.isBlank(propertyHome) && !Files.isDirectory(Paths.get(propertyHome)))
            return propertyHome;
